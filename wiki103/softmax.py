"""Adapted from torch.nn.modules.adaptive"""
...
class AdaptiveLogSoftmaxWithLoss(Module):
    ...
    * :attr:`head_bias` if set to True, adds a bias term to the 'head' of the
    ...
    def __init__(self, in_features: int, n_classes: int, cutoffs: Sequence[int],
                 div_value: float = 4., head_bias: bool = False, device=None, dtype=None) -> None:
    ...
    def forward(self, input_: Tensor, target_: Tensor) -> _ASMoutput:
        ...
        output = input.new_zeros(batch_size)  # , dtype=torch.float32)
        ...
        for i in range(len(cutoff_values) - 1):
            ...
            if i == 0:
                ...
                # gather_inds[row_indices] = target[target_mask]  # alt
                ...
                # gather_inds[row_indices] = cluster_index  # alt
                cluster_logprob = log_softmax(cluster_output, dim=1)
                local_logprob = cluster_logprob.gather(1, relative_target.unsqueeze(1)).squeeze(1)
                # local_logprob = cluster_logprob[torch.arange(len(relative_target)),
                #                                 relative_target]   # alt
                output.index_copy_(0, row_indices, local_logprob)
                # output[row_indices] = local_logprob  # alt
